...
def _ensure_db_initialized():
    ...
    _c.execute('''CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE, password TEXT, role TEXT, full_name TEXT, is_active BOOLEAN DEFAULT 1)''')    for u_name, u_sym in [('قطعة','قطعة'),('لتر','لتر'),('كيلو','كجم'),('متر','متر'),('كرتونة','كرتونة'),('رول','رول'),('زجاجة','زجاجة'),('علبة','علبة'),('كيس','كيس')]:
...
if choice == "📊 لوحة التحكم":
                ...
                for _, row in edited_df.iterrows():
                    if pd.isna(row["id"]):
                        ...
                        if exists:
                            ...
                            continue
                        unit_id = unit_dict.get(row["unit_text"], units[0]["id"] if units else 1)
    ...
    if st.button("🔁 إعادة حساب جميع الأرصدة (من الحركات)"):
        with st.spinner("جاري إعادة حساب الأرصدة..."):
            recalculate_all_balances()
        st.success("تمت إعادة حساب جميع الأرصدة بنجاح.")
        st.rerun()
